...
/**
 * A {@code HighOrderCrfChunker} implements chunking based on a high-order CRF
 * over string sequences, a tokenizer factory, and a tag to chunk coder/decoder.
 *
 ...
 * data structures as for high-order CRFs, nodes and edges.  For chunkers,
 ...
 * <h3>Training</h3>
 *
 * The static {@code estimate()} method is used to train a high order
 ...
 * passed off to the high-order CRF estimator method.  Feature extractors
 * are the same as for a high-order CRF, with one for nodes and one for
 * edges.  The tags passed in to these feature extractors will be
 * determiend by the tag-chunk codec.  The remaining inputs are
 * identical to those for high-order CRFs; see the method documentation for
 * more information.
 *
 * <h3>Decoding</h3>
 *
 * A high-order CRF chunker implements all three chunker interfaces in
 ...
 * <h3>Serialization</h3>
 *
 * High-order CRF chunkers are serializable if their contained tokenizer
 * factories and codecs are serializable.  The chunker read back in
 * will be of this class, {@code HighOrderCrfChunker}, with components
 * derived from serialization and deserialization.
 *
 * <h3>Thread Safety</h3>
 *
 * The high-order CRF chunker class is thread safe if the tokenizer
 * factory and tag/chunk coder/decoder are thread safe.
 */
public class HighOrderCrfChunker implements Chunker, ConfidenceChunker, NBestChunker, Serializable {

    static final long serialVersionUID = -2244399751558084581L;

    private final HighOrderCrf<String> mCrf;
    private final TokenizerFactory mTokenizerFactory;
    private final TagChunkCodec mCodec;

    /**
     * Construct a chunker based on the specified high-order conditional
     ...
     * and vice-versa.
     */
    public HighOrderCrfChunker(HighOrderCrf<String> crf,
     ...
     */
    public HighOrderCrf<String> crf() {
        return mCrf;
    }
    ...
    public Iterator<ScoredObject<Chunking>> nBest(char[] cs, int start, int end, int maxResults) {
        PreTagging preTagging = preTag(cs,start,end);
        List<String> tokens = preTagging.mTokens;
        Iterator<ScoredTagging<String>> it = mCrf.tagNBest(tokens,maxResults);
    ...
    public Iterator<ScoredObject<Chunking>> nBestConditional(char[] cs, int start, int end, int maxResults) {
        PreTagging preTagging = preTag(cs,start,end);
        List<String> tokens = preTagging.mTokens;
        Iterator<ScoredTagging<String>> it = mCrf.tagNBestConditional(tokens,maxResults);
        return new IteratorWrapper(it,preTagging,cs,start,end,mCodec);
    }
    ...
    /**
     * Return the high-order CRF-based chunker estimated from the specified
     * corpus, which is converted to a tagging corpus using the
     * specified coder/decoder and tokenizer factory, then passed to
     * the high-order CRF estimate method along with the rest of the
     * arguments.
     *
     * <p>Estimation is based on regularized stochastic gradient
     * descent.  See {@link
     * HighOrderCrf#estimate(Corpus,HighOrderCrfFeatureExtractor,boolean,int,boolean,boolean,RegressionPrior,int,AnnealingSchedule,double,int,int,Reporter)}
     ...
     * token sequences for the underlying high-order CRF.
     * @param featureExtractor Feature extractor for the underlying high-order CRF.
     ...
     * extracted features. 
     * @param prior Prior to use to regularize the underlying high-order
     * CRF estimates.
     ...
     * the corpus.
     */
    public static HighOrderCrfChunker estimate(Corpus<ObjectHandler<Chunking>> chunkingCorpus,
                                               TagChunkCodec codec,
                                               TokenizerFactory tokenizerFactory,
                                               HighOrderCrfFeatureExtractor<String> featureExtractor,
        ...
        reporter.info("Training high-order CRF chunker");
        reporter.info("Converting chunk corpus to tag corpus using codec.");
        Corpus<ObjectHandler<Tagging<String>>> taggingCorpus = new TagCorpus(chunkingCorpus,codec);
        HighOrderCrf<String> crf = HighOrderCrf.estimate(taggingCorpus,
                                featureExtractor,
                                addInterceptFeature,
                                minFeatureCount,
                                cacheFeatureVectors,
                                ...
                                maxEpochs,
                                reporter);
        return new HighOrderCrfChunker(crf,tokenizerFactory,codec);
    }
    ...
    static int[] toArray(List<Integer> xs) {
        int len = xs.size();
        int[] ys = new int[len];
        for (int i = 0; i < len; ++i) ys[i] = xs.get(i);
        return ys;
    }
    ...
    static class IteratorWrapper implements Iterator<ScoredObject<Chunking>> {
        ...
        public ScoredObject<Chunking> next() {
            ...
            Chunking chunking = toChunking(tagging,mPreTagging,mCs,mStart,mEnd,mCodec);
    ...
    static class Serializer extends AbstractExternalizable {
        ...
        private final HighOrderCrfChunker mChunker;
        public Serializer() {
            this(null);
        }
        public Serializer(HighOrderCrfChunker chunker) {
            mChunker = chunker;
        }
        public Object read(ObjectInput in) throws IOException, ClassNotFoundException {
            @SuppressWarnings("unchecked")
            HighOrderCrf<String> crf = (HighOrderCrf<String>) in.readObject();
            @SuppressWarnings("unchecked")
            TokenizerFactory factory = (TokenizerFactory) in.readObject();
            @SuppressWarnings("unchecked")
            TagChunkCodec codec = (TagChunkCodec) in.readObject();
            return new HighOrderCrfChunker(crf,factory,codec);
        }
