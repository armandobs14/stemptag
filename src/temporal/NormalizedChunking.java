...
public class NormalizedChunking implements Chunking, Iterable<Chunk> {

	private final String mString;
    private final Set<NormalizedChunk> mChunkSet = new LinkedHashSet<NormalizedChunk>();
     ...
     */
    public NormalizedChunking(CharSequence cSeq) {
     ...
     */
    public NormalizedChunking(char[] cs, int start, int end) {
     ...
     */
    public void addAll(Collection<NormalizedChunk> chunks) {
        for (NormalizedChunk next : chunks)
            add(next);
    }
     ...
     */
    public void add(NormalizedChunk chunk) {
    ...
    public boolean equals(Object that) {
        return (that instanceof NormalizedChunking)
            ? equal(this,(NormalizedChunking)that)
            : false;
    }
    ...
    static final NormalizedChunk[] EMPTY_CHUNK_ARRAY = new NormalizedChunk[0];
     ...
     */
    public static boolean equal(NormalizedChunking chunking1, NormalizedChunking chunking2) {
     ...
     */
    public static int hashCode(NormalizedChunking chunking) {
     ...
     * position.
     */
    public static boolean overlap(NormalizedChunk chunk1, NormalizedChunk chunk2) {
     ...
     * character sequence.
     */
    public static NormalizedChunking merge(NormalizedChunking chunking1, NormalizedChunking chunking2) {
        ...
        NormalizedChunking chunking = new NormalizedChunking(chunking1.charSequence().toString());
        NormalizedChunk[] chunks1 = sortedChunks(chunking1);
        NormalizedChunk[] chunks2 = sortedChunks(chunking2);
        int pos1 = 0;
        NormalizedChunk lastChunk = null;
    ...
    static boolean overlap(NormalizedChunk[] chunks, int pos, NormalizedChunk lastChunk) {
    ...
    // +pos2
    static boolean isBefore(NormalizedChunk[] chunks1, int pos1, NormalizedChunk[] chunks2, int pos2) {
        return pos1 < chunks1.length
            && chunks1[pos1].end() <= chunks2[pos2].start();
    }

    static boolean overlapOneWay(NormalizedChunk chunk1, NormalizedChunk chunk2) {
        return chunk1.start() <= chunk2.start()
            && chunk2.start() < chunk1.end();
    }

    static final NormalizedChunk[] sortedChunks(NormalizedChunking chunking) {
        Set<Chunk> chunkSet = chunking.chunkSet();
        NormalizedChunk[] chunks = chunkSet.toArray(EMPTY_CHUNK_ARRAY);
