...
public class CRFFeatureExtractor implements com.aliasi.crf.ChainCrfFeatureExtractor<String>, Serializable {
    ...
    public static HashSet<String> readDictionary ( File data ) throws IOException {
    	...
    	String aux;
    	while ((aux=reader.readLine())!=null) if(aux.trim().length() > 0) set.add(aux.toLowerCase().trim());
    	return set;
    }
    ...
    class ChunkerFeatures extends ChainCrfFeatures<String> {
        ...
        public Map<String,? extends Number> nodeFeatures(int n) {
			...
			if ( timeNames!=null ) {
            	if (timeNames.contains(token.toLowerCase())) feats.set("TNAMES",1.0);
            	if (!bos && timeNames.contains(prevToken.toLowerCase())) feats.set("TNAMES_PREV",1.0);
            	if (!eos && timeNames.contains(nextToken.toLowerCase())) feats.set("TNAMES_NEXT",1.0);
			}
