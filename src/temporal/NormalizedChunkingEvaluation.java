...
public class NormalizedChunkingEvaluation extends ChunkingEvaluation{
	...
	public NormalizedChunkingEvaluation() {
        /* do nothing */
    }
	...
	public NormalizedChunkingEvaluation perTypeEvaluation(String chunkType) {
		NormalizedChunkingEvaluation evaluation = new NormalizedChunkingEvaluation();
	...
	public void addCase(Chunking referenceChunking,
		...
		Set<NormalizedChunk> refSet = unscoredNormalizedChunkSet(referenceChunking);
		Set<NormalizedChunk> respSet = unscoredNormalizedChunkSet(responseChunking);

		for (NormalizedChunk respChunk : respSet) {
		
		Iterator<NormalizedChunk> it = refSet.iterator();
		...
		while(it.hasNext()){
			...
			if (nChunk.equals(respChunk)){
				...
				if (nChunk.getNormalized() !=null)
					System.out.println("teste: "+nChunk.getNormalized());
				it.remove();
				flag = true;
				break;
			}	
	...
	static Set<NormalizedChunk> unscoredNormalizedChunkSet(Chunking chunking) {
        Set<NormalizedChunk> result = new HashSet<NormalizedChunk>();
        for (Chunk chunk : chunking.chunkSet())
            result.add((NormalizedChunk)chunk);
        return result;
    }
